...
static inline bool SetBit(volatile unsigned long *array, int bit) {
    bool flag;
...
seekRecord_t<skey_t, sval_t>* wfrbt<skey_t, sval_t, RecMgr>::insseek(thread_data_t<skey_t, sval_t>* data, skey_t key, int op) {
...
template <typename skey_t, typename sval_t, class RecMgr>
seekRecord_t<skey_t, sval_t>* wfrbt<skey_t, sval_t, RecMgr>::delseek(thread_data_t<skey_t, sval_t>* data, skey_t key, int op) {
    node_t<skey_t, sval_t> * gpar = NULL; // last node (ancestor of parent on access path) whose child pointer field is unmarked
    node_t<skey_t, sval_t> * par = data->rootOfTree;
    node_t<skey_t, sval_t> * leaf;
    node_t<skey_t, sval_t> * leafchild;
...
template <typename skey_t, typename sval_t, class RecMgr>
seekRecord_t<skey_t, sval_t>* wfrbt<skey_t, sval_t, RecMgr>::secondary_seek(thread_data_t<skey_t, sval_t>* data, skey_t key, seekRecord_t<skey_t, sval_t>* sr) {
...
template <typename skey_t, typename sval_t, class RecMgr>
sval_t wfrbt<skey_t, sval_t, RecMgr>::search(thread_data_t<skey_t, sval_t>* data, skey_t key) {
    ...
    while (cur != NULL) {
        lastKey = cur->key;
        lastNode = cur;
        cur = (key < lastKey ? (node_t<skey_t, sval_t> *)get_addr(cur->child.AO_val1) : (node_t<skey_t, sval_t> *)get_addr(cur->child.AO_val2));
    }
    if (key == lastKey) {
#ifndef BASELINE
...
template <typename skey_t, typename sval_t, class RecMgr>
int wfrbt<skey_t, sval_t, RecMgr>::help_conflicting_operation(thread_data_t<skey_t, sval_t>* data, seekRecord_t<skey_t, sval_t>* R) {
...
template <typename skey_t, typename sval_t, class RecMgr>
int wfrbt<skey_t, sval_t, RecMgr>::inject(thread_data_t<skey_t, sval_t>* data, seekRecord_t<skey_t, sval_t>* R, int op) {
...
template <typename skey_t, typename sval_t, class RecMgr>
sval_t wfrbt<skey_t, sval_t, RecMgr>::wfrbt_insert(thread_data_t<skey_t, sval_t>* data, skey_t key, sval_t value) {
    int injectResult;
    int fasttry = 0;
    while (true) {
        seekRecord_t<skey_t, sval_t>* R = insseek(data, key, INS);
        fasttry++;
        if (R->leafKey == key) {
            if (fasttry == 1) {
                return R->leafValue;
...
template <typename skey_t, typename sval_t, class RecMgr>
bool wfrbt<skey_t, sval_t, RecMgr>::delete_node(thread_data_t<skey_t, sval_t>* data, skey_t key) {

    int injectResult;
    while (true) {
        seekRecord_t<skey_t, sval_t>* R = delseek(data, key, DEL);
        if (R == NULL) {
            return false;
        }
...
template <typename skey_t, typename sval_t, class RecMgr>
int wfrbt<skey_t, sval_t, RecMgr>::perform_one_insert_window_operation(thread_data_t<skey_t, sval_t>* data, seekRecord_t<skey_t, sval_t>* R, skey_t newKey, sval_t value) {
    node_t<skey_t, sval_t> * newInt;
    node_t<skey_t, sval_t> * newLeaf;
    //		if(data->recycledNodes.empty()){		
    node_t<skey_t, sval_t> * allocedNodeArr = (node_t<skey_t, sval_t> *)tree_malloc::allocnode(2 * sizeof (struct node_t<skey_t, sval_t>)); // new pointerNode_t[2];
...
template <typename skey_t, typename sval_t, class RecMgr>
int wfrbt<skey_t, sval_t, RecMgr>::perform_one_delete_window_operation(thread_data_t<skey_t, sval_t>* data, seekRecord_t<skey_t, sval_t>* R, skey_t key) {
    // mark sibling.
    AO_t pS;
    if (R->isLeftL) {
        // L is the left child of P
