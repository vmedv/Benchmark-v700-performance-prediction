...
class SAITBuilder : public TreeBuilder<SAIT<int, Value>, Value> {
public:
    using Tree = SAIT<int, Value>;

    SAITBuilder(int left, int right) : left_(left), right_(right) {}
...
void StressTest(const tree_tests::StressTestConfig<int> &config) {
    tree_tests::StressTest(new SAITBuilder<tree_tests::ValueType>(config.GetMinKey(), config.GetMaxKey() + 1), config)();
}

TEST_CASE("insert_delete") {
    tree_tests::UnitTest(new SAITBuilder<int>(-100, 100), tree_tests::kInsertDeleteAction)();
}
