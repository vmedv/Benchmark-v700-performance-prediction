    ...
    for nrq in 0 1; do
        ...
        for ((trial=0;trial<$trials;++trial)) ; do
            ...
            if (($counting==1)); then
                ...
                echo "env LD_PRELOAD=../lib/libjemalloc-5.0.1-25.so TREE_MALLOC=../lib/libjemalloc-5.0.1-25.so numactl --interleave=all $cmd" > $fname
                env LD_PRELOAD=../lib/libjemalloc-5.0.1-25.so TREE_MALLOC=../lib/libjemalloc-5.0.1-25.so numactl --interleave=all $cmd >> $fname
