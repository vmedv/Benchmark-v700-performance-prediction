                ...
                for alg in brown_ext_ist_lf brown_ext_abtree_lf bronson_pext_bst_occ natarajan_ext_bst_lf ; do
                    for n in $thread_counts ; do
                        if ((counting)); then
                            ...
                            cmd="LD_PRELOAD=../../../lib/libjemalloc.so timeout $timeout numactl --interleave=all time ../../bin/ubench_${alg}.alloc_new.reclaim_debra.pool_none.out $args"
                            echo "cmd=$cmd" > $f
                            echo "step=$step" >> $f
                            echo "fname=$f" >> $f
                            eval $cmd >> $f 2>&1
                            maxres=`../grep_maxres.sh $f`
                            echo "maxresident=$maxres" >> $f
