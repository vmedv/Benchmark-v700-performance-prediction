...
def plot_rc(row, col, series, x, y, data=None, where='', series_styles=None, plot_func=sns.lineplot, facetgrid_kwargs=dict()):
...
def init_for_jupyter(user_experiment_python_file):
    ...
    args = do_all('{} --production --no-compile --no-run --no-createdb'.format(user_experiment_python_file).split(' '))
