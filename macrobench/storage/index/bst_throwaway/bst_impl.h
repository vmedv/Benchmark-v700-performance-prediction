...
template<class K, class V, class Compare, class RecManager>
const V bst<K,V,Compare,RecManager>::doInsert(const int tid, const K& key, const V& val, bool onlyIfAbsent) {
    V result = NO_VALUE;
...
template<class K, class V, class Compare, class RecManager>
const V bst<K,V,Compare,RecManager>::insertIfAbsent(const int tid, const K& key, const V& val) {
...
//        cout<<"trying to retire scx records"<<std::endl;
        for (int j=0;j<highestIndexReached;++j) {
            ...
            if (scxRecordsSeen[j] != LLX_RETURN_IS_LEAF && !IS_VERSION_NUMBER(scxRecordsSeen[j])) {
                ...
                tryRetireSCXRecord(tid, scxRecordsSeen[j], nodes[j]);
            }
        }
