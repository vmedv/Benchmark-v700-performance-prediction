...
#else
    const static int MAX_NODES = DEGREE+2; // watch for padding issues with DEGREE != 16
#endif
...
//    volatile long padding[1]; // pad to size that is a multiple of 8, but is NOT a multiple of any higher power of 2
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
    volatile char space_for_alignment[64]; // add extra space for manual alignment up to 64 bytes
#endif
...
//    volatile long padding[1]; // pad to size that is a multiple of 8, but is NOT a multiple of any higher power of 2
    ...
    template <class Compare>
    inline bslack_Node<DEGREE,K> * getChild(const K& key, Compare cmp) {
...
private:
    ...
    #define arraycopy(src, srcStart, dest, destStart, len, srcSize, destSize) \
        assert(srcStart >= 0); \
        assert(destStart >= 0); \
        assert(len == 0 || srcStart + len <= srcSize); \
        assert(len == 0 || destStart + len <= destSize); \
...
private:
    ...
    void deallocate(const int tid, bslack_SCXRecord<DEGREE,K> * p);
    void retire(const int tid, bslack_SCXRecord<DEGREE,K> * p);
    bslack_Node<DEGREE,K>* allocateNode(const int tid);
    void deallocate(const int tid, bslack_Node<DEGREE,K> * p);
    void retire(const int tid, bslack_Node<DEGREE,K> * p);
...
public:
    ...
    bslack_Node<DEGREE,K> * debug_getEntryPoint() { return entry; }
