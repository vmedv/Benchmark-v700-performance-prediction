...
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
extern __thread unsigned nodeAllocations;
extern __thread unsigned scxRecordAllocations;
#endif

//static unsigned long long MarsagliaXOR(unsigned long long seed) {
//    seed ^= seed << 12;
//    seed ^= seed >> 25;
//    seed ^= seed << 27;
...
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
    const unsigned alignment_padding = 8 * (scxRecordAllocations++ % 8);
    newop = (bslack_SCXRecord<DEGREE,K> *) (((char *) newop) + alignment_padding);
    newop->offset = alignment_padding;
#endif
...
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
    const unsigned alignment_padding = 8 * (nodeAllocations++ % 8);
    newnode = (bslack_Node<DEGREE,K> *) (((char *) newnode) + alignment_padding);
    newnode->offset = alignment_padding;
#endif
...
template <int DEGREE, typename K, class Compare, class RecManager>
void bslack<DEGREE,K,Compare,RecManager>::deallocate(const int tid, bslack_Node<DEGREE,K> * p) {
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
    p = (bslack_Node<DEGREE,K> *) (((char *) p) - p->offset);
#endif

    recordmgr->deallocate(tid, p);
}

template <int DEGREE, typename K, class Compare, class RecManager>
void bslack<DEGREE,K,Compare,RecManager>::deallocate(const int tid, bslack_SCXRecord<DEGREE,K> * p) {
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
    p = (bslack_SCXRecord<DEGREE,K> *) (((char *) p) - p->offset);
#endif
...
template <int DEGREE, typename K, class Compare, class RecManager>
void bslack<DEGREE,K,Compare,RecManager>::retire(const int tid, bslack_Node<DEGREE,K> * p) {
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
    p = (bslack_Node<DEGREE,K> *) (((char *) p) - p->offset);
#endif
...
template <int DEGREE, typename K, class Compare, class RecManager>
void bslack<DEGREE,K,Compare,RecManager>::retire(const int tid, bslack_SCXRecord<DEGREE,K> * p) {
#ifdef ABTREE_FORCE_ALIGNMENT_SPREAD
    p = (bslack_SCXRecord<DEGREE,K> *) (((char *) p) - p->offset);
#endif
...
//            gp = p;
        ...
        if (keyIndex < l->getKeyCount() && l->keys[keyIndex] == key) {
            ...
            arraycopy(l->keys, 0, n->keys, 0, l->getKeyCount(), DEGREE, DEGREE);
            arraycopy(l->ptrs, 0, n->ptrs, 0, l->getABDegree(), DEGREE, DEGREE);
            ...
            if (l->getKeyCount() < b) {
                ...
                // create new node(s)
                bslack_Node<DEGREE,K>* n = allocateNode(tid);
                arraycopy(l->keys, 0, n->keys, 0, keyIndex, DEGREE, DEGREE);
                assert(keyIndex >= 0);
                assert(l->getKeyCount()-keyIndex == 0 || l->getKeyCount()+1 <= DEGREE);
                arraycopy(l->keys, keyIndex, n->keys, keyIndex+1, l->getKeyCount()-keyIndex, DEGREE, DEGREE);
                n->keys[keyIndex] = key;
                arraycopy(l->ptrs, 0, n->ptrs, 0, keyIndex, DEGREE, DEGREE);
                assert(l->getABDegree()-keyIndex == 0 || l->getABDegree()+1 <= DEGREE);
                arraycopy(l->ptrs, keyIndex, n->ptrs, keyIndex+1, l->getABDegree()-keyIndex, DEGREE, DEGREE);
                n->ptrs[keyIndex] = (bslack_Node<DEGREE,K>*) value;
                n->leaf = l->leaf;
                n->marked = false;
                n->scxPtr = DUMMY;
                ...
                arraycopy(l->keys, 0, keys, 0, keyIndex, DEGREE, DEGREE+1);
                arraycopy(l->keys, keyIndex, keys, keyIndex+1, l->getKeyCount()-keyIndex, DEGREE, DEGREE+1);
                keys[keyIndex] = key;
                arraycopy(l->ptrs, 0, ptrs, 0, keyIndex, DEGREE, DEGREE+1);
                arraycopy(l->ptrs, keyIndex, ptrs, keyIndex+1, l->getABDegree()-keyIndex, DEGREE, DEGREE+1);
                ...
                arraycopy(keys, 0, left->keys, 0, size1, DEGREE+1, DEGREE);
                arraycopy(ptrs, 0, left->ptrs, 0, size1, DEGREE+1, DEGREE);
                left->leaf = true;
                left->marked = false;
                left->scxPtr = DUMMY;
                ...
                arraycopy(keys, size1, right->keys, 0, size2, DEGREE+1, DEGREE);
                arraycopy(ptrs, size1, right->ptrs, 0, size2, DEGREE+1, DEGREE);
                right->leaf = true;
                right->marked = false;
                right->scxPtr = DUMMY;
...
const pair<void*,bool> bslack<DEGREE,K,Compare,RecManager>::erase(const int tid, const K& key) {
    ...
    while (true) {
        ...
        if (keyIndex == l->getKeyCount() || l->keys[keyIndex] != key) {
            ...
            arraycopy(l->keys, 0, n->keys, 0, keyIndex, DEGREE, DEGREE);
            arraycopy(l->keys, keyIndex+1, n->keys, keyIndex, l->getKeyCount()-(keyIndex+1), DEGREE, DEGREE);
            arraycopy(l->ptrs, 0, n->ptrs, 0, keyIndex, DEGREE, DEGREE);
            arraycopy(l->ptrs, keyIndex+1, n->ptrs, keyIndex, l->getABDegree()-(keyIndex+1), DEGREE, DEGREE);
            n->leaf = true;
            n->marked = false;
            n->scxPtr = DUMMY;
...
bool bslack<DEGREE,K,Compare,RecManager>::fixWeightViolation(const int tid, bslack_Node<DEGREE,K>* viol) {
    ...
    while (true) {
        ...
        if (size <= b) {
            ...
            bslack_Node<DEGREE,K>* n = allocateNode(tid);
            arraycopy(p->ptrs, 0, n->ptrs, 0, ixToL, DEGREE, DEGREE);
            arraycopy(l->ptrs, 0, n->ptrs, ixToL, l->getABDegree(), DEGREE, DEGREE);
            assert(ixToL >= 0);
            assert(p->getABDegree()-(ixToL+1) == 0 || ixToL+l->getABDegree() + p->getABDegree()-(ixToL+1) <= DEGREE);
            arraycopy(p->ptrs, ixToL+1, n->ptrs, ixToL+l->getABDegree(), p->getABDegree()-(ixToL+1), DEGREE, DEGREE);
            arraycopy(p->keys, 0, n->keys, 0, ixToL, DEGREE, DEGREE);
            arraycopy(l->keys, 0, n->keys, ixToL, l->getKeyCount(), DEGREE, DEGREE);
            assert(p->getKeyCount()-ixToL == 0 || ixToL+l->getKeyCount() + p->getKeyCount()-ixToL <= DEGREE);
            arraycopy(p->keys, ixToL, n->keys, ixToL+l->getKeyCount(), p->getKeyCount()-ixToL, DEGREE, DEGREE);
            ...
            arraycopy(p->ptrs, 0, ptrs, 0, ixToL, DEGREE, 2*DEGREE);
            arraycopy(l->ptrs, 0, ptrs, ixToL, l->getABDegree(), DEGREE, 2*DEGREE);
            arraycopy(p->ptrs, ixToL+1, ptrs, ixToL+l->getABDegree(), p->getABDegree()-(ixToL+1), DEGREE, 2*DEGREE);
            arraycopy(p->keys, 0, keys, 0, ixToL, DEGREE, 2*DEGREE);
            arraycopy(l->keys, 0, keys, ixToL, l->getKeyCount(), DEGREE, 2*DEGREE);
            arraycopy(p->keys, ixToL, keys, ixToL+l->getKeyCount(), p->getKeyCount()-ixToL, DEGREE, 2*DEGREE);
            ...
            bslack_Node<DEGREE,K>* left = allocateNode(tid);
            arraycopy(keys, 0, left->keys, 0, size1-1, 2*DEGREE, DEGREE);
            arraycopy(ptrs, 0, left->ptrs, 0, size1, 2*DEGREE, DEGREE);
            ...
            bslack_Node<DEGREE,K>* right = allocateNode(tid);
            arraycopy(keys, size1, right->keys, 0, size2-1, 2*DEGREE, DEGREE);
            arraycopy(ptrs, size1, right->ptrs, 0, size2, 2*DEGREE, DEGREE);
            right->leaf = false;
            right->marked = false;
            right->scxPtr = DUMMY;
            right->searchKey = keys[size1];
...
bool bslack<DEGREE,K,Compare,RecManager>::fixDegreeOrSlackViolation(const int tid, bslack_Node<DEGREE,K>* viol) {
    ...
    while (true) {
        ...
        pGrandDegree = 0;
        for (int i=0;i<p->getABDegree();++i) {
            arraycopy(pChildren[i]->keys, 0, keys, pGrandDegree, pChildren[i]->getKeyCount(), DEGREE, DEGREE*DEGREE);
            arraycopy(pChildren[i]->ptrs, 0, ptrs, pGrandDegree, pChildren[i]->getABDegree(), DEGREE, DEGREE*DEGREE);
            pGrandDegree += pChildren[i]->getABDegree();
        ...
        for (int i=0;i<nodesWithCeil;++i) {
            ...
            arraycopy(keys, degreePerNodeCeil*i, child->keys, 0, degreePerNodeCeil - !pChildrenAreLeaves, DEGREE*DEGREE, DEGREE);
            arraycopy(ptrs, degreePerNodeCeil*i, child->ptrs, 0, degreePerNodeCeil, DEGREE*DEGREE, DEGREE);
        ...
        for (int i=0;i<nodesWithFloor;++i) {
            bslack_Node<DEGREE,K>* child = allocateNode(tid);
            arraycopy(keys, degreePerNodeCeil*nodesWithCeil+degreePerNodeFloor*i, child->keys, 0, degreePerNodeFloor - !pChildrenAreLeaves, DEGREE*DEGREE, DEGREE);
            arraycopy(ptrs, degreePerNodeCeil*nodesWithCeil+degreePerNodeFloor*i, child->ptrs, 0, degreePerNodeFloor, DEGREE*DEGREE, DEGREE);
            child->leaf = pChildrenAreLeaves;
            child->marked = false;
            child->scxPtr = DUMMY;
        ...
        if (gp == entry && numberOfNewChildren == 1) {
            ...
            bslack_Node<DEGREE,K>* n = allocateNode(tid);
            arraycopy(newChildren, 0, n->ptrs, 0, numberOfNewChildren, DEGREE, DEGREE);
...
bslack_SCXRecord<DEGREE,K>* bslack<DEGREE,K,Compare,RecManager>::llx(const int tid, bslack_Node<DEGREE,K>* r, bslack_Node<DEGREE,K> ** snapshot) {
    ...
    if (state & bslack_SCXRecord<DEGREE,K>::STATE_ABORTED || ((state & bslack_SCXRecord<DEGREE,K>::STATE_COMMITTED) && !r->marked)) {
        ...
        if (snapshot != NULL) {
            arraycopy(r->ptrs, 0, snapshot, 0, r->getABDegree(), DEGREE, DEGREE);
        }
        LWSYNC;
